...
#################################################
#  EAGO EOI TESTS -> RUN THIS W/ "EE" AND "IE"  #
#################################################
using EAGO
@mtkcompile o = KineticParameterEstimation()
model = Model(optimizer_with_attributes(EAGO.Optimizer, "relative_tolerance" => 1e-4))
set_attribute(model, "time_limit", 3600*24*14.0) # 14 day limit
decision_vars(o) # Displays: x_Z(t), x_Y(t), x_D(t), x_B(t), x_A(t), k_2f, k_3f, k_4
# FIRST, create discretized state decision variables
N = Int(floor((tspan[2] - tspan[1])/tstep))+1
V = length(unknowns(o))
