...
@testset "InfiniteOpt ODE Model" begin
    ...
    V = length(unknowns(system))
    InfiniteOpt.@infinite_parameter(model, τ ∈ tspan, num_supports = N, derivative_method = OrthogonalCollocation(2))
    InfiniteOpt.@variable(model, -75.0 <= z[1:V] <= 150.0, Infinite(τ))
    pL = [10.0, 10.0, 0.001]
    pU = [1200.0, 1200.0, 40.0]
    JuMP.@variable(model, pL[i] <= p[i=1:3] <= pU[i])

    EOptInterface.register_odesystem(model, system, τ)

    JuMP.@objective(model, Min, sum((intensity(z[5](i), z[4](i), z[3](i)) - data.intensity[findfirst(==(i), data.time)])^2 for i in supports(τ)))
    JuMP.optimize!(model)
